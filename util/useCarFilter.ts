...
//   const uniqueLocations = [...new Set(carsData.map((car) => car.location))]
//   const uniqueRatings = [...new Set(carsData.map((car) => Math.round(car.rating)))]
...
//       (filter.locations.length === 0 || filter.locations.includes(car.location)) &&
//       (car.price >= filter.priceRange[0] && car.price <= filter.priceRange[1]) &&
//       (filter.ratings.length === 0 || filter.ratings.includes(Math.round(car.rating))) &&
...
const useCarFilter = (carsData: Car[]) => {
  ...
  const uniqueRatings = Array.from(
    new Set(carsData.map((car) => Number(car.rating.toFixed(1))))
  ).sort((a, b) => a - b)
